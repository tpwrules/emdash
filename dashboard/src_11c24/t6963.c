...
#define set_nRESET(to) Chip_GPIO_WritePortBit(LPC_GPIO, 1, 11, to)
#define set_FS(to) Chip_GPIO_WritePortBit(LPC_GPIO, 1, 10, to)

#define set_DB(to) do { LPC_GPIO[0].DATA[0x3FC] = to << 2; } while(0)
#define set_DB_output(to) do {\
    if (to) LPC_GPIO[0].DIR |= 0x3FC; \
    else LPC_GPIO[0].DIR &= ~(0x3FC); } while(0)
